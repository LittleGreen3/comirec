...
class Model(object):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, seq_len, flag="DNN"):
        ...
        with tf.compat.v1.name_scope('Embedding_layer'):
            ...
            self.mid_embeddings_bias = tf.compat.v1.get_variable("bias_lookup_table", [n_mid], initializer=tf.compat.v1.zeros_initializer(), trainable=False)
    ...
    def call(self, item_his_emb, item_eb, mask):
        with tf.compat.v1.variable_scope('bilinear'):
            if self.bilinear_type == 0:
                ...
                item_emb_hat = tf.keras.layers.Dense(self.dim * self.num_interest, activation=None, use_bias=False)(item_his_emb)
            else:
